...
RAG-часть сохраняет indexing foundation: загрузку документов, fixed-size и structure-aware
chunking, Ollama/hash embeddings, JSON/SQLite indexes и cosine retrieval. После Day 22 она также
содержит baseline и grounded RAG QA modes, чистые prompt builders, DeepSeek/fake LLM adapters,
контрольный eval set и JSON/Markdown comparison reports.

Fake provider и hash embeddings используются только для deterministic tests, CI и offline smoke.
Они проверяют orchestration, но не считаются качественным RAG-результатом. Основной Day 22 demo
использует semantic embeddings Ollama `nomic-embed-text` и реальную DeepSeek LLM.
...
| `ai_advent_agent.rag` | Ingestion, chunking, embeddings, local stores, retrieval, prompts, LLM adapters, QA и reports. |
...
- `ai_advent_agent.rag` — reusable ingestion/indexing/retrieval и grounded QA API без
  day-specific CLI.
