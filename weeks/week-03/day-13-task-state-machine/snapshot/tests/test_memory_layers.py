...
class Day11MemoryLayersTest(unittest.TestCase):
    ...
    def test_profile_prompt_is_inserted_before_memory_layers(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_dir = Path(tmp)
            client = FakeClient()
            agent = build_memory_agent(tmp_dir, client)
            agent.create_profile("concise_engineer")
            agent.set_profile_field("style", "кратко и технически")
            agent.remember_long("project", "AI Advent")
            agent.remember_working("task", "assistant personalization")
            agent.remember_short("проверить prompt order")

            response = agent.ask("Что делать дальше?")

            sent = client.calls[0]
            self.assertEqual(sent[0]["content"], "system")
            self.assertIn("User profile preferences", sent[1]["content"])
            self.assertIn("Long-term memory", sent[2]["content"])
            self.assertIn("Working memory", sent[3]["content"])
            self.assertEqual(
                response.token_report.prompt_assembly_order,
                [
                    "system",
            ...
            self.assertTrue(response.token_report.profile_active)
            self.assertEqual(response.token_report.active_profile_name, "concise_engineer")
            self.assertGreater(response.token_report.profile_prompt_tokens_estimated, 0)

    def test_empty_profile_is_not_inserted_into_prompt(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            client = FakeClient()
            agent = build_memory_agent(Path(tmp), client)
            agent.create_profile("empty")

            response = agent.ask("Без профиля")

            contents = [message["content"] for message in client.calls[0]]
            self.assertFalse(any("User profile preferences" in content for content in contents))
            self.assertFalse(response.token_report.profile_active)
