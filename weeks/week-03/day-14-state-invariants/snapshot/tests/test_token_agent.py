...
from ai_advent_agent.agent import STEP_BY_STEP_SUFFIX, ContextOverflowError, SimpleAgent
...
class Day8AgentTest(unittest.TestCase):
    def test_agent_saves_messages_and_token_report(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(
                [item["role"] for item in payload["messages"]], ["system", "user", "assistant"]
            )
    ...
    def test_agent_restores_context_on_restart(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(
                [item["role"] for item in sent_messages], ["system", "user", "assistant", "user"]
            )
