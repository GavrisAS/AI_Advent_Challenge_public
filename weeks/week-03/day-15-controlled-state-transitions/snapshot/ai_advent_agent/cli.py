...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    invariants_file_default = os.getenv("AI_ADVENT_INVARIANTS_FILE") or str(DEFAULT_INVARIANTS_FILE)
    invariant_events_file_default = os.getenv("AI_ADVENT_INVARIANT_EVENTS_FILE") or str(
        DEFAULT_INVARIANT_EVENTS_FILE
    )
    ...
    parser.add_argument(
        "--invariants-file",
        type=Path,
        default=Path(invariants_file_default),
        help="JSON-файл hard invariants. По умолчанию .agent_context/invariants.json.",
    )
    parser.add_argument(
        "--invariant-events-file",
        type=Path,
        default=Path(invariant_events_file_default),
        help="JSONL-файл invariant events. По умолчанию .agent_context/invariant_events.jsonl.",
    )
    parser.add_argument(
        "--task-state-file",
        type=Path,
...
def build_agent(args: argparse.Namespace) -> SimpleAgent:
    ...
    invariant_store = None if args.no_persist else JsonInvariantStore(args.invariants_file)
    invariant_event_store = (
        None if args.no_persist else InvariantEventStore(args.invariant_events_file)
    )
    ...
    return SimpleAgent(
        ...
        invariant_event_store=invariant_event_store,
...
def main(argv: list[str] | None = None) -> None:
    ...
    print("Day 14. Агент со state invariants")
    ...
    print(f"Файл invariants: {agent.invariants_path or 'сохранение отключено'}")
    print(f"Файл invariant events: {agent.invariant_events_path or 'логирование отключено'}")
