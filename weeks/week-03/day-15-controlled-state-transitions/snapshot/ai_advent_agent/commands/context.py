...
def print_config(agent: SimpleAgent) -> None:
    ...
    print(
        ...
        f"  user_profiles_file: {agent.user_profiles_path or 'disabled'}\n"
        f"  profile_events_file: {agent.profile_events_path or 'disabled'}\n"
        f"  task_state_file: {agent.task_state_path or 'disabled'}\n"
        f"  task_events_file: {agent.task_events_path or 'disabled'}\n"
...
def print_token_report(report: TokenReport) -> None:
    ...
    if report.profile_active:
        print(
            "profile_active: true\n"
            f"active_profile_name: {report.active_profile_name}\n"
            f"profile_fields_count: {format_number(report.profile_fields_count)}\n"
            "profile_prompt_tokens_estimated: "
            f"{format_number(report.profile_prompt_tokens_estimated)}"
        )
    if report.invariants_active:
        print(
            "invariants_active: true\n"
            f"invariants_count: {format_number(report.invariants_count)}\n"
            "invariants_prompt_tokens_estimated: "
            f"{format_number(report.invariants_prompt_tokens_estimated)}"
        )
    if report.invariant_conflict:
        print(
            "invariant_conflict: true\n"
            f"invariant_conflict_count: {format_number(report.invariant_conflict_count)}"
        )
    ...
    elif report.prompt_assembly_order:
        print(f"prompt_assembly_order: {', '.join(report.prompt_assembly_order)}")
    if report.task_state_active:
        print(
            "task_state_active: true\n"
            f"task_stage: {report.task_stage or '-'}\n"
            f"task_done: {report.task_done}\n"
            f"task_paused: {report.task_paused}\n"
            f"task_plan_approved: {report.task_plan_approved}\n"
            f"task_validation_passed: {report.task_validation_passed}\n"
            "task_allowed_next_stages: "
            f"{', '.join(report.task_allowed_next_stages) or '-'}\n"
            f"task_last_transition_allowed: {report.task_last_transition_allowed}\n"
            "task_prompt_tokens_estimated: "
            f"{format_number(report.task_prompt_tokens_estimated)}"
        )
