...
class SimpleAgent:
    ...
    def __init__(
        ...
        *,
        ...
        self.task_last_transition_allowed: bool | None = None
    ...
    def reset(self, *, save: bool = True, clear_reports: bool = True) -> None:
        ...
        self.task_state = TaskState()
        self.task_last_transition_allowed = None
    ...
    def start_task(self, title: str) -> None:
        ...
        if self.task_state.active and not self.task_state.done:
            raise ValueError(
                "Активная задача уже есть. Завершите её или используйте /task reset --yes"
            )
    ...
    def set_task_stage(self, stage: str) -> TaskTransitionResult:
        """Legacy guarded alias for controlled task transition."""

        return self.transition_task(stage)

    def transition_task(self, stage: str) -> TaskTransitionResult:
        """Move task through the controlled lifecycle."""

        self._require_active_task()
        result = apply_task_transition(self.task_state, stage)
        self.task_last_transition_allowed = result.allowed
        if result.allowed:
            self.save_task_state()
            self._append_task_event(action="transition_task", transition=result)
        else:
            self._append_task_event(action="invalid_transition", transition=result)
        return result

    def approve_task_plan(self) -> None:
        """Approve planning output before execution can start."""

        self._require_active_task()
        if self.task_state.paused:
            raise ValueError("Задача на паузе. Сначала выполните: /task resume")
        if self.task_state.stage != "planning":
            raise ValueError("План можно утвердить только на этапе planning")
        self.task_state.plan_approved = True
        self.task_state.updated_at = now_utc()
        self.save_task_state()
        self._append_task_event(action="approve_plan")

    def pass_task_validation(self) -> None:
        """Mark validation as passed before task completion."""

        self._require_active_task()
        if self.task_state.paused:
            raise ValueError("Задача на паузе. Сначала выполните: /task resume")
        if self.task_state.stage != "validation":
            raise ValueError("Validation можно отметить пройденной только на этапе validation")
        self.task_state.validation_passed = True
        self.task_state.updated_at = now_utc()
        self.save_task_state()
        self._append_task_event(action="pass_validation")
    ...
    def advance_task(self) -> TaskTransitionResult:
        ...
        return self.transition_task(target_stage)
    ...
    def complete_task(self) -> TaskTransitionResult:
        """Complete active task."""

        result = self.transition_task("done")
        if result.allowed:
            self._append_task_event(action="complete_task", transition=result)
        return result
    ...
    def _build_preflight_report(
        ...
        *,
        ...
        return TokenReport(
            ...
            task_plan_approved=self.task_state.plan_approved,
            task_validation_passed=self.task_state.validation_passed,
            task_allowed_next_stages=allowed_next_task_stages(self.task_state),
            task_last_transition_allowed=self.task_last_transition_allowed,
