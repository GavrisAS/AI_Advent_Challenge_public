...
class SimpleAgent:
    ...
    def _prepare_messages_for_request(
        ...
        *,
        ...
        request_messages, memory_metadata = self._build_request_messages(
            persistent_messages,
            user_message,
        )
    ...
    def _build_request_messages(
        ...
        if memory_messages:
            ...
            memory_metadata.assembly_order = [
                "system",
                *(["user_profile"] if profile_message is not None else []),
                *memory_metadata.assembly_order,
                *self._non_memory_prompt_order(
                    request_messages[insert_at + len(memory_messages) :]
                ),
                "current_user",
            ]
        request_messages.append(user_message.copy())
        if not memory_messages:
            memory_metadata.assembly_order = self._non_memory_prompt_order(request_messages)
        if profile_message is not None:
        ...
        return request_messages, memory_metadata
